...
### Second pass (after the owner's review)
...
| **Directional hit flinches** (`animations.js`, `takeHit`) | `hitFlinchL`/`R` (shoved sideways, near arm flung) and `hitFlinchBack` (folded forward) picked by the blow's angle; the torso impulse rides the matching axis. Measured: hand up 3.3 units, head pushed 0.9 the right way. |
| **Per-gait intro crouch** (`defClipVariants`) | Non-biped gaits take the shared crouch at a fraction, arms left alone (arm clip values are absolute — the humanoid "arms back" hung jerry's claws through the road). Intro depth under the floor: jerry 2.22 → 0.40, fenrir 1.72 → 0.49, tritone 1.34 → 0.50, cranky → 0.66. |
...
| **`npm run check`** | 27 node tests (roster ids ↔ handlers, clip names, gait schema, text ids, tuning rates, level round-trip) + the pure-node tools, run in `deploy.yml` before the build. |
| **One tools harness** | `tools/lib/browser.mjs` (`PW_CHROMIUM` overrides the Chromium path); 162 tools migrated by codemod, four run end-to-end after. |
| **Kinetic impact retuned** (`fighter.js`) | `KNOCKDOWN_KICK` 9.5 → 60, `CLOSING_MIN` 5 → 12, damage bonus cap +110% → +50%, and `HEAVY_SHOVE`: mass past 1.5 adds 20 u/s of closing speed to every blow, so a heavy frame floors a scout with no run-up. Over the 272 ordered pairs, floored from a standstill 0 → 10 (titanus/colossus/cranky on viper, wraith, saurion), at a walk 241 → 68, at a sprint 263 → 105. A body that has just hit the floor carries 0.3 s of iframes (no relaunch off the ground). |
| **Balance nudges** | Serpent storm pins each victim once per cast; raptor-pack minions throw no specials; wraith's ghost walk cooldown 9 → 7; viper's and wraith's guard leak 0.20 → 0.15. The other weapon outliers in 2.1 D were left as they are, by decision. |
| **Arena pass** | Frozen's lakes and river are ICE (steering gain and dash bleed scale by a per-frame grip: stopping distance 2.1 → 8.8 units, a right-angle turn 0.22 → 0.85 s). Sky Terrace has three VOID drops between its decks (fall 0.6 s, 15% hp, respawn on the far pad with iframes; the skybridges span them) and Orbital three LOWGRAV pads (jump apex 2.8 → 10.9) plus a freezing cryo-tank blast. Every arena paints its plaza ring. Foundry, harbor and scrapyard build 36 sites instead of 26–30; harbor has one lobed basin and container nests; harbor and scrapyard keep their facade colour (`tintLerp` 0.4). Snow drifts no longer bloom white; ice has a slate shore. The per-round rotation excludes an arena's twin. |
| **CPU floor sense** (`ai.js`) | One body length ahead is probed for lava/acid/void, prop bodies and live fuel tanks; the CPU takes the clear perpendicular or holds, and walks out of lava it is standing in. Volcano, veteran, 60 s: titanus lava time 9.7% → 0.7%, viper 6.4% → 0.9%, prop contact frames 56/160 → 0. |
| **TRAINING mode** (`src/game/training.js`) | A TRAINING tile on arena select (never automatic). Every seated player plays at once on Uptown; CPU slots are passive dummies that face you and walk back to their pad; no clock, KOs respawn on the pad, ult charges infinite, ammo refills, hp regenerates after 4 s. Each seat gets its own checklist beside its plate — MOVE → JUMP → HOVER → LIGHT → HEAVY → BLOCK → DASH → RANGED → SPECIAL → ULTIMATE → TAUNT — naming that seat's buttons, ticking off the fighter's real state. `node tools/training.mjs` asserts 31 things (order of ticks, respawn at full hp, dummy never attacks over 20 s, two- and three-seat HUDs do not overlap). Dev: `?battle=uptown&p1=titanus&p2=viper&training=1`. |
...
- ~~The CPU has no hazard or prop awareness~~ — DONE (second pass). Original: it walks down lava lanes after its target and bumps fuel tanks. On volcano and foundry fights against the CPU are decided by the floor. Sample `onLane/onPatch` a body-length ahead and steer around `lava/acid`; treat `propBodies` as obstacles. **High on two arenas / medium.**
...
1. ~~Clip-to-clip crossfade~~ — done (second pass).
2. ~~Directional hit reactions~~ — done.
3. ~~Per-gait intro~~ — done; tritone's hand and cranky's pincer still graze the limit (0.50 / 0.66 vs 0.40–0.45).
...
Items 1–7 and 9 below are DONE in the second pass (see section 1); 8 (landmark sightlines) and the per-arena notes in 10 are still open.
...
6. ~~Onboarding prompts~~ / 7. ~~Practice mode~~ — done as the TRAINING tile (second pass). Still open from this item: a "MASH A" prompt over a downed human the first two times.

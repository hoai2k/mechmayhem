...
| **Getting up is free, and either button does it** | Both JUMP and DASH now answer a knockdown — on the floor with something standing over you the dash is what a hand reaches for, and B did nothing at all while downed before (its coil path refuses every state but normal/attack). The dash is read as a HOLD: what gets you up is the controller as it is now, never what it was at the moment you fell, so a player already holding B — every player who was sprinting — does not have to notice, release and press again. Nothing on the path touches stamina: the economy charges for a dodge you chose, and being floored is not a choice. Measured from a 3-unit knockdown with an EMPTY tank: both buttons spring at vy 11.3 with 0.87 s of iframes, and when the invulnerability ends he is 10.41 units away against the attacker's 6.05 of reach — the bar meanwhile refills 0.019 → 0.192, never spent. **Consequence to watch:** a player who holds B is never pinned by a knockdown at all (he springs within ~3 frames), and a flipped cranky starts his righting roll at once instead of lying stranded for `ROLLOVER_DOWN_TIME` |
...
### 7.5 Fifth pass — the now-playing transport, and konga's salvo

| Done | What |
|---|---|
| **The now-playing chip is a transport on hover** | ⏮ and ⏭ open out of the chip and the title slides to sit between them; at rest it is the same one-line readout. Mouse only by design — every pad button is spoken for by the fight. BACK is two buttons: the first press REWINDS, a second within `BACK_AGAIN` steps to the previous song (the first press always rewinds, whatever the playhead says, so the button does the same thing every time you reach for it). Stepping back makes the song you left the NEXT one, so the two buttons are each other's undo. A nav press while PAUSED auditions at `PREVIEW_MIX` of the bus (measured 0.110 against 0.314) and the unpause hands it back to full; a hidden tab still stops it. `node tools/scratch/npchip.mjs` — 15 checks with a real mouse |
| **A second barrel aims from where it is** (`world.js dirFrom`) | KONGA's shoulder salvo under a target lock left **31° wide at 8 units**, 16° at 14, 10.8° at 22 and 6.8° at 30 — the reported "in target mode my bullets went totally to the side". It is not obstacles and not the barrel angles: the aim is solved once from the PRIMARY muzzle and every other barrel was handed that same DIRECTION, which is parallel fire from a point several units to one side, so the miss is pure parallax and grows as the target gets CLOSER. His right pod (the primary) was within 3° at every range the whole time. Barrels now converge on the point the primary's own line reaches at the target's range — which removes the parallax and adds nothing, since the point comes off the AIM rather than off the enemy, so a line that misses by three units still misses by three out of every barrel. Konga worst 34.3° → **4.5°** (the authored per-round scatter); off-target spread also tightened for frogger (18.5° → 3.4°) and vulcan, and nullbot and jerry are unchanged. The primary barrel is unchanged by construction — its own solve returns the direction it came in with. `node tools/scratch/kongaim.mjs <mech>` |

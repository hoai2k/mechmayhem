...
export class GameAudio {
  ...
  // ------------------------------------------------------------------------
  // THE RECORDED SET (public/sfx/, generated by tools/sfxgen.mjs)
  //
  // A recording SHADOWS the synthesized sound of the same name: `play('hit')`
  // is the only call site either way, so nothing in combat code knows or
  // cares which one it got. Three properties this is built around —
  //
  //   * NOTHING WAITS. The manifest is fetched in the background and each
  //     file is decoded on FIRST USE; until a sound has landed, its synth
  //     version plays. So a cold load is the game as it has always been, and
  //     it fills in as the fight goes on rather than gating the start of it.
  //   * A MISSING FILE IS NOT AN ERROR. Anything the set doesn't cover keeps
  //     its synth version forever, which is what lets the set land one sound
  //     at a time.
  //   * MULTI-TAKE FILES GO THROUGH THE SLICER. `sliced: true` in the
  //     manifest means the file holds several takes separated by silence; the
  //     existing loadSliced/playSlice path finds them and plays a different
  //     one each trigger, so a footstep never machine-guns.
  // ------------------------------------------------------------------------

  /** Fetch the manifest. Safe to call repeatedly; failure is silent. */
  async loadSfxBank(base = 'sfx/') {
    if (this._bank) return this._bank;
    this._bank = {};                       // claim it, so a second call is a no-op
    try {
      const url = new URL(`${base}manifest.json`, document.baseURI).href;
      const res = await fetch(url);
      if (!res.ok) return this._bank;
      const man = await res.json();
      this._bankBase = base;
      this._bank = man;
    } catch (e) { /* no recorded set: the synth keeps the game */ }
    return this._bank;
  }

  /** Decode one sound, once. Returns the entry or null while it is in flight. */
  _sample(name) {
    const entry = this._bank?.[name];
    if (!entry) return null;
    const got = this._samples[name];
    if (got && got !== 'loading') return got === 'failed' ? null : got;
    if (got === 'loading') return null;
    this._samples[name] = 'loading';
    const base = this._bankBase || 'sfx/';
    const url = new URL(base + entry.file, document.baseURI).href;
    if (entry.sliced) {
      // the slicer owns the buffer for a multi-take file
      this.loadSliced(name, url).then((n) => {
        this._samples[name] = n > 0 ? { sliced: true, category: entry.category } : 'failed';
      }).catch(() => { this._samples[name] = 'failed'; });
      return null;
    }
    (async () => {
      try {
        ...
        this._samples[name] = { buffer, category: entry.category };
      } catch (e) { this._samples[name] = 'failed'; }
    })();
    return null;
  }

  /** Play a decoded one-shot on the SFX bus at its category's level. */
  _playSample(s, name, vol, rate) {
    const gain = vol * sfxGain(s.category);
    if (s.sliced) return this.playSlice(name, { vol: gain, rate });
    const ctx = this._init();
    if (!ctx || ctx.state !== 'running') return false;
    const src = ctx.createBufferSource();
    src.buffer = s.buffer;
  ...
  play(name, opts = {}) {
    try {
      ...
      // a recording plays INSTEAD of the synth — and only when it is already
      // decoded, so nothing is ever delayed waiting for one.
      //
      // A MECH MAY HAVE ITS OWN VOICE: `<mech>_<name>` (fenrir_taunt) shadows
      // the shared sound for that mech alone, so a wolf howls where everyone
      // else beeps. One naming rule, no table — dropping fenrir_taunt.mp3 in
      // is the whole of adding one.
      if (CONFIG.sfxSamples) {
        const own = opts.mech ? `${opts.mech}_${name}` : null;
        if (own && this._bank?.[own]) {
          const s = this._sample(own);
          if (s && this._playSample(s, own, v, p)) return;
        }
        const s = this._sample(name);
        if (s && this._playSample(s, name, v, p)) return;
      }

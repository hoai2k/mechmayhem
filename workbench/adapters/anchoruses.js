...
//     or off the barrel named by roster `primaryMuzzle` when the mech carries
...
const ROLE = {
  muzzleR: 'Primary spawn point: fireRanged reads it for EVERY ranged type (unless roster `primaryMuzzle` names the other barrel), and muzzle() falls back to it whenever a named anchor is missing.',
  muzzleL: 'Second barrel — the alternating-side weapons and volleys read it, and it is the PRIMARY on a mech whose roster sets primaryMuzzle to it. When absent those shots all come from muzzleR.',
...
export function anchorUses(def, name, available) {
  ...
  const mv = def?.moves || {};
  if (mv.ranged?.type) {
    // roster `primaryMuzzle` re-points the primary barrel for this mech only
    const spec = RANGED[mv.ranged.type];
    consider('ranged', `Ranged — ${mv.ranged.name || mv.ranged.type}`,
      spec && def.primaryMuzzle ? { ...spec, primary: def.primaryMuzzle } : spec);
  }
